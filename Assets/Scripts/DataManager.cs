...
public class DataManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        header = new string[] { "Difficulty", "Item", "Score", "Miss", "Combo", "MaxCombo", "Time", "Date" };             // ヘッダーを作成
    ...
    public void OnClickOutputButton()
    {
        ...
        foreach(string dataStr in PlayerController.dataStrs)                 // csvファイルの中身を書き込み
        {
            sw.WriteLine(dataStr);
        }
        sw.Close();
#elif UNITY_WEBGL
        FileDownLoad(string.Join(",", header) + "\n" + string.Join("\n", PlayerController.dataStrs), fileName);
#endif
